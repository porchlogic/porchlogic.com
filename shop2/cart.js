...
function addItemToCart(item) {
    ...
    const incomingGlyphData =
        item && item.glyphData !== undefined
            ? JSON.parse(JSON.stringify(item.glyphData))
            : null;
...
function renderCartItems() {
    ...
    cartItems.forEach((item) => {
        ...
        if (item.id === 'm8_plate_1') {
            ...
            m8Wrapper.className = 'config-stack';
            ...
            glyphRow.className = 'config-row';

            const glyphLeft = document.createElement('div');
            glyphLeft.className = 'config-left';
            glyphLeft.innerHTML = `
            ...
            const glyphRight = document.createElement('div');
            glyphRight.className = 'config-right';
            glyphRight.innerHTML = `
            ...
            glyphRow.appendChild(glyphLeft);
            glyphRow.appendChild(glyphRight);
            ...
            liveRow.className = 'config-row';

            const liveLeft = document.createElement('div');
            liveLeft.className = 'config-left';
            liveLeft.innerHTML = `
                <label class="live-label" for="${liveCheckboxId}">
            ...
            liveRight.className = 'config-right';
            liveRight.innerHTML = `
            ...
            liveRow.appendChild(liveLeft);
            liveRow.appendChild(liveRight);

            m8Wrapper.appendChild(glyphRow);
            m8Wrapper.appendChild(liveRow);
...
function attachMoundGrid(uid, editorEl, existingData, options = {}) {
    ...
    const instance = createMoundGrid(
        ...
        (data) => {
            ...
            } else if (!options.onDataChange) {
                updateGlyphThumbnail(uid, glyphCopy);
            } else if (!options.onThumbnailUpdate) {
                updateGlyphThumbnail(uid, glyphCopy);
            }
        }
    );
